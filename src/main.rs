...
fn scroll_left<SPI: blocking::spi::write::Default<u8>, CS: OutputPin>(
    ...
    for i in 0..8 {
        ...
        for col in 0..8 {
            ...
            display.write_raw_byte(0, (col + 1) as u8, data).unwrap();
            delay.delay_ms(1);
        }
    }
}

fn show_message<SPI: blocking::spi::write::Default<u8>, CS: OutputPin>(
    display: &mut MAX7219<SpiConnectorSW<SPI, CS>>,
    delay: &mut Delay,
    msg: &str,
    font: &[[u8; 8]],
    ...
    let mut prev = ' ';
    for curr in msg.chars() {
        scroll_left(display, delay, prev as u8, curr as u8, font);
        prev = curr;
    }
